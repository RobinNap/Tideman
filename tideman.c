...
int main(int argc, string argv[])
{
    ...
    {
        // ranks [i] is voter's its preference
...
// update preferences given one voters rank
void record_preferences(int ranks[])
{
    for (int i = 0; i < candidate_count; i++)
    {
...
// record pairs of candidates where one is prefferd over other
void add_pairs(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
...
// sort pairs in decreasing order by strenght of victory
void sort_pairs(void)
{
...
// lock pairs into the candidates graph in order, without creating cycles
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        locked[pairs[i].winner][pairs[i].loser] = true;
        // check for cycle in this directed graph starting with its candidates
